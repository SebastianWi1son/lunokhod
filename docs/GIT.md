...
**双段式（2026-09-16 定，agent 参与的提交一律照此）**：一条 commit 由两段组成，**缺一不可**。

```
构建各组件统一装配层          ← 第一段：用户的一句话（给自己判断进度用）

—— 以下为 agent 补充（完整变更清单，供下一个 agent / 用户接手）——

【新增】chassis_loop：装配层库…
【改动】构建与 CI…
【修复】…
【文档】…
【已知问题】…                 ← 没做完 / 有已知缺陷的，必须明说
```

| 段 | 谁写 | 要求 |
|---|---|---|
| 第一段 | **用户** | 一句话；agent **不许改写他的措辞** |
| 第二段 | **agent** | 要能回答「改了什么 / 为什么 / 影响谁 / 有什么已知问题」，**不必翻 diff 就能接手**；分节词固定用 `【新增】【改动】【修复】【文档】【CI】【已知问题】` |

> 为什么：commit 是**唯一活过会话的东西**。用户要一句话看进度，下一个 agent 要完整上下文 ——
> 一个读者要摘要，一个读者要全貌，合并成一条消息用两段满足。
